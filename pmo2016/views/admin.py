...
class AdminView(CommonView):
    _sub_list = ["backcomment", "battle", "default", 'stall', 'login', 'info']
    name = "admin"
    admin = None
    ...
    def _backcomment_post_return(comment_id=None):
        response = redirect("pmo2016:admin", sub='backcomment')
        if comment_id:
    ...
    def _backcomment_post(self, request):
        ...
        if main_comment.email and request.POST.get('send_email') == 'send':
            ...
            if page is None:
                raise Http404
            p = loader.get_template('pmo2016/mails/comment_back.html').render({
    ...
    def _battle_post(self, request):
        ...
        send_mail(
            ...
            html_message=loader.get_template('pmo2016/mails/battle_validated.html').render({
                'player': current, 'base_url': settings.BASE_URL,
                'message': request.POST.get('message'), 'signup_id': '%03d' % current.pk
            })
        )
        response = redirect("pmo2016:admin", sub='battle')
    ...
    def _info_post(self, request):
        ...
        if ncn:
            ...
            self.admin.save()
        return redirect("pmo2016:admin", sub='info')
    ...
    def _stall_post(self, request):
            ...
            if notice:
                ...
                if request.POST.get('send_email') == 'send':
                    send_mail(
                        ...
                        settings.EMAIL_HOST_USER, [current.email], fail_silently=False,
                        html_message=loader.get_template('pmo2016/mails/seller_notice.html').render({
                            'seller': current, 'base_url': settings.BASE_URL,
                        })
                    )
            response = redirect("pmo2016:admin", sub='stall')
            ...
            if seller_id:
                ...
                if request.POST.get('send_email') == 'send':
                    send_mail(
                        '%s%s通知' % (settings.EMAIL_SUBJECT_PREFIX, "摊位" if current.is_stall else "寄卖"), "",
                        settings.EMAIL_HOST_USER, [current.email], fail_silently=False,
                        html_message=loader.get_template('pmo2016/mails/seller_sellerid.html').render({
                            'seller': current, 'base_url': settings.BASE_URL,
                        })
                    )
            response = redirect("pmo2016:admin", sub='stall')
        ...
        send_mail(
            ...
            settings.EMAIL_HOST_USER, [current.email], fail_silently=False,
            html_message=loader.get_template('pmo2016/mails/seller_validated.html').render({
                'seller': current, 'base_url': settings.BASE_URL,
    ...
    def post(self, request, sub=None):
        if sub == 'login':
            ...
            if user is None or not any(user.groups.filter(name='PmoAdminGroup')):
                response = redirect("pmo2016:admin", sub='login')
