...
from pmo2016.forms import BattleForm
...
class RegisterView(CommonView):
    _sub_list = ["battle", "stall", "signupin", "wrong"]
    _err_dict = {
        "1": "邮箱已通过验证，请登录",
        "2": "注销成功"
    }
    ...
    def get(self, request, sub=None, *args, **kwargs):
        ...
        if sub == 'stall':
            if not request.user.is_authenticated():
                return redirect("pmo2016:register", sub='signupin')
            ...
            if 'item_id' in request.GET:
                ...
                if item.count() == 1:
                    return render(
                        request,
                        'pmo2016/register/stall/itemform.html',
                        {
                            'item': item[0],
            ...
            if 'page' in request.GET:
                ...
                return render(request, 'pmo2016/register/stall/itemtable.html', {
                    'items': items,
                    'page': page
                })
            is_stall = sub == 'stall'
            if seller.is_stall == is_stall:
                ...
                kwargs.update({
                    'is_stall': is_stall,
                    'correct_url': reverse('pmo2016:register', kwargs={'sub': 'consign' if is_stall else 'stall'})
                })
                sub = 'wrong'
        elif sub == 'signupin':
            kwargs.update({
