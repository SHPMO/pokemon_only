...
class SignupView(ApiView):
    @staticmethod
    def send_validate_mail(seller, validate_code, pmo):
        send_mail(
    ...
    def post(self, request, *args, **kwargs):
        ...
        s_seller = Seller.objects.filter(email=signup.cleaned_data['email'], pmo=pmo)
        if s_seller.count() > 0:
        ...
        self.send_validate_mail(seller, validate_code, pmo)
...
class ValidateView(View):
    ...
    def get(request, *args, **kwargs):
        try:
            vc = ValidateCode.objects.get(code=request.GET["validate_code"], pmo=request.GET["pmo"])
        ...
        vc.save()
        vc.seller.save()
        response = redirect("dashboard:register", sub='signupin')
